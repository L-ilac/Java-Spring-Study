...
public class ValidationItemControllerV2 {
    ...
    public String addItemV1(@ModelAttribute Item item, BindingResult bindingResult,
        ...
        if (item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            bindingResult.addError(new FieldError("item", "price", "가격은 1000~ 1000000원 까지 허용합니다."));
        }
        ...
        if (item.getPrice() != null && item.getQuantity() != null) {
            ...
            if (resultPrice < 10000) {
                bindingResult.addError(new ObjectError("item",
                        "가격 * 수량의 합은 10000원 이상이어야 합니다. 현재값 = " + resultPrice));
            }
        }
        ...
        if (bindingResult.hasErrors()) {
            ...
            // ! bindingresult는 모델에 직접 담아주지 않아도 알아서 담겨짐
            return "validation/v2/addForm";
        }
    ...
    public String addItemV2(@ModelAttribute Item item, BindingResult bindingResult,
        ...
        if (item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
            bindingResult.addError(new FieldError("item", "price", item.getPrice(), false, null, null,
                    "가격은 1000~ 1000000원 까지 허용합니다."));
        }

        if (item.getQuantity() == null || item.getQuantity() >= 9999) {
            bindingResult.addError(
                    new FieldError("item", "quantity", item.getQuantity(), false, null, null, "수량은 최대 9999개까지 허용합니다."));
        }
    ...
    public String addItemV3(@ModelAttribute Item item, BindingResult bindingResult,
        ...
        // * 검증로직
        if (!StringUtils.hasText(item.getItemName())) {
            // ! messagesource를 통해 code와 arguments로 오류 메세지를 errors.properties에서 갖고온다.
            bindingResult.addError(
    ...
    public String addItemV5(@ModelAttribute Item item, BindingResult bindingResult,
        ...
        if (bindingResult.hasErrors()) {
            ...
            // ! errors는 모델에 직접 담아주지 않아도 알아서 담겨짐
            return "validation/v2/addForm";
        }

...
//        QMember m = QMember.member;
        ...
        Member findByQuerydsl = query.select(member)
                .from(member)
...
//                .having()
    ...
    public void join(){
        List<Member> result = query
                .selectFrom(member) // 조인의 기준이 되는 테이블
                .join(member.team, team) // 기준 테이블에 조인하고자하는 테이블 + 별칭으로 사용할 Q타입
//                .leftJoin()
//                .rightJoin()
//                .innerJoin()
    ...
    public void theta_join(){
        ...
        List<Member> result = query
                .select(member) // 조인의 기준이 되는 테이블
                .from(member, team)
                .where(member.username.eq(team.name))
                .fetch();

        // ! 실제 DB에 날라가는 SQL은 cross join 수행
    ...
    @Test
    public void join_on_no_relationship(){
        em.persist(new Member("teamA"));
        em.persist(new Member("teamB"));
        em.persist(new Member("teamC"));

        List<Tuple> result = query
                .select(member, team) // 조인의 기준이 되는 테이블
                .from(member)
                .leftJoin(team) // ! leftJoin(member.team, team) 이 아님
    ...
    public void noFetchJoin(){
        ...
        boolean loaded1 = emf.getPersistenceUnitUtil().isLoaded(findMember.getTeam());
        assertThat(loaded1).as("페치 조인 미적용").isFalse();
    }
    ...
    public void FetchJoin(){
        ...
        Member findMemberWithTeam = query
                .selectFrom(member)
                .join(member.team, team).fetchJoin()
                .where(member.username.eq("member1"))
                .fetchOne();

        boolean loaded2 = emf.getPersistenceUnitUtil().isLoaded(findMemberWithTeam.getTeam());
    ...
    @Test
    public void findDtoDifferentAlias(){
        QMember memberSub = new QMember("memberSub");
...
//                        ExpressionUtils.as(member.age,"age")
    ...
    private List<Member> searchMember1(String usernameCond, Integer ageCond) {
        ...
        // ! BooleanBuilder 생성할 때 초기조건으로 넣어줄 수도 있음.
...
//                .where(usernameEq(usernameCond), ageEq(ageCond))
    ...
    public void bulkUpdate(){
        ...
        // ! 벌크연산은 영속성 컨텍스트를 무시하고 바로 DB에 쿼리가 나가기때문에, 영속성컨텍스트와 DB의 데이터가 싱크가 안맞는다.
        // * repeatable read -> JPA는 영속성 컨텍스트에 데이터가 존재하면, DB에서 동일한 id의 데이터를 읽어오더라도 무시한다.

        em.flush();
        em.clear();

        // * 위의 flush와 clear로 날려준 후에, 데이터를 읽어온다.
...
//                        member.username)))
    ...
    @Test
    public void searchWhereParameter(){
        MemberSearchCondition condition = new MemberSearchCondition();
